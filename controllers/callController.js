...
exports.handleRegistration = async (req, res) => {
  ...
  if (digit === '3') {
    ...
    // Connect to Vapi AI assistant WITHOUT disconnecting
    twiml.say('Please wait while we connect you to our AI assistant for registration help.');
    
    // Use Twilio's <Dial> to connect to a conference where VAPI will join
    const dial = twiml.dial();
    dial.conference('VAPI-ASSISTANT-' + Date.now(), {
      startConferenceOnEnter: true,
      endConferenceOnExit: false,
      statusCallback: '/voice/vapi-webhook'
    });
    
    // Initiate VAPI call to join the same conference
    setTimeout(() => {
      handleVapiCall(callerNumber);
    }, 2000);
